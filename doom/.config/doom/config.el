...
;; Recalculate tables on save
(add-hook 'org-mode-hook
          (lambda ()
            (add-hook 'before-save-hook #'org-table-recalculate-buffer-tables nil 'local)))

;; Custom functions
;;
(defun reload-dotemacs-file ()
  "Инициализация конфигурационных файлов Emacs без перезагрузки."
  (interactive)
  (load-file user-init-file))

(defun org-id-complete-link (&optional arg)
  "Автодополнение для id: в org-insert-link"
  (interactive "P")
  (concat "id:" (org-id-get-with-outline-path-completion)))

(defun org-link-describe (link desc)
  "Автоподстановка поля 'Описание' для ссылки в 'org-insert-id'."
  (if (string-prefix-p "id:" link)
      (let* ((id (string-remove-prefix "id:" link))
             (marker (org-id-find id 'marker))
             (heading (org-entry-get marker "ITEM")))
        heading)
    desc))

(defun org-id-get-from-link (link)
  "Получение ID из ссылки, напр. [[id:...][...]]"
  (when (string-match "\\[\\[id:\\([^]]+\\)\\]" link)
    (match-string 1 link)))

;; User defined functions
;;
(defun xddi/pfck (link weight prop)
  "Рассчет КБЖУ для указанного ингредиента 'link' и указанного веса 'weight'.
Базовое значение для 100 грамм берется свойства карточки ингредтиента 'prop'."
  (let* ((id (org-id-get-from-link link))
         (marker (org-id-find id 'marker))
         (source-value (org-entry-get marker prop))
         (source-value-num (string-to-number source-value))
         (weight-num (string-to-number weight)))
    (/ (* weight-num source-value-num) 100.00)))
